...
export class DataSource {
   ...
   */
  protected getNodeIdsForLinks(links: GraphLink[]): (string | number)[] {
   ...
   * @returns The links
   */
  protected getLinksForNodesWithIds(
    ids: (string | number)[],
    links: GraphLink[],
  ...
  getNeighborhood(
    ...
    if (allowedNeighborDepth === 0)
      return {
        nodes: [node],
        links: this.getLinksForNodesWithIds(
          [node._id],
          graphData.links,
          LinkDirections.both
        ),
    ...
    while (neighborLevel < allowedNeighborDepth) {
      ...
      const allPrimaryLinks: GraphLink[] = this.getLinksForNodesWithIds(
        primaryNodesIds,
        graphData.links,
      ...
      const secondaryNodesIds: (string | number)[] = this.getNodeIdsForLinks(
        shownLinks
      ).filter(
        // eslint-disable-next-line no-loop-func
        (nId) =>
          !primaryNodesIds.includes(nId) && !checkedNodesIds.includes(nId)
      );

      const allSecondaryLinks: GraphLink[] = this.getLinksForNodesWithIds(
        secondaryNodesIds,
        graphData.links,
        LinkDirections.both
      );
...
export class AirtableDataSource extends DataSource {
  ...
  private hideUnselected(allowedNeighborDepth: number = 1): void {
    ...
    //   return;
    // }
    const visibleGraphData: AppGraphData = this.getNeighborhood(
      selectedNode,
