...
export class DataSource {
  ...
  removeNodeTypeOfName(name: string): void {
    ...
    this.links = this.links.filter(
      l => l._linkType === undefined || !linkTypesToDelete.includes(l._linkType)
    )
...
export class AirtableDataSource extends DataSource {
  ...
  constructor(
    ...
    const initBaseId: string | undefined = process.env.REACT_APP_DEFAULT_BASE_ID
  ...
  async load(): Promise<void> {
    ...
    nodeType.tableMeta =
      this.tableMeta.find(tm => tm.name === nodeType.name) || nodeType.tableMeta
  ...
  private hideUnlinkedNodes(): void {
    ...
    if (!this.graphConfig.nodeTypes.some(nt => nt.config.hideUnlinked)) return
    ...
    const linkedNodeIds: (string | number)[] = isStruct
      ...
      : [
          ...new Set(
            this.links.map(n => [n.source as string, n.target as string]).flat()
          ),
        ]
